...
VulkanPbr::VulkanPbr(void *native_window, std::string asset_path, uint32_t width, uint32_t height) : VulkanExampleBase(native_window, asset_path.c_str(), width, height),
...
void VulkanPbr::loadAssets() {
    ...
    //std::string sceneFile = assetpath + "/models/Box/glTF-Embedded/Box.gltf";
...
void VulkanPbr::preparePipelines() {
    ...
    shaderStages = {
            loadShader(device, assetpath + "/shaders/skybox.vert.spv", VK_SHADER_STAGE_VERTEX_BIT),
            loadShader(device, assetpath + "/shaders/skybox.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT)
    };
    ...
    // PBR pipeline
    shaderStages = {
            loadShader(device, assetpath + "/shaders/pbr.vert.spv", VK_SHADER_STAGE_VERTEX_BIT),
            loadShader(device, assetpath + "/shaders/pbr_khr.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT)
    };
    ...
    VK_CHECK_RESULT(
            vkCreateGraphicsPipelines(device, pipelineCache, 1, &pipelineCI, nullptr, &pipelines.pbrDoubleSided));
    ...
    VK_CHECK_RESULT(
            vkCreateGraphicsPipelines(device, pipelineCache, 1, &pipelineCI, nullptr, &pipelines.pbrAlphaBlend));
...
void VulkanPbr::generateBRDFLUT() {
    ...
    shaderStages = {
            loadShader(device, assetpath + "/shaders/genbrdflut.vert.spv", VK_SHADER_STAGE_VERTEX_BIT),
            loadShader(device, assetpath + "/shaders/genbrdflut.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT)
    };
    VkPipeline pipeline;
...
void VulkanPbr::generateCubemaps() {
    ...
    for (uint32_t target = 0; target < PREFILTEREDENV + 1; target++) {
        ...
        shaderStages[0] = loadShader(device, assetpath + "/shaders/filtercube.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
        switch (target) {
            case IRRADIANCE:
                shaderStages[1] = loadShader(device, assetpath + "/shaders/irradiancecube.frag.spv",
                                             VK_SHADER_STAGE_FRAGMENT_BIT);
                break;
            case PREFILTEREDENV:
                shaderStages[1] = loadShader(device, assetpath + "/shaders/prefilterenvmap.frag.spv",
                                             VK_SHADER_STAGE_FRAGMENT_BIT);
                break;
        };
        VkPipeline pipeline;
...
void VulkanPbr::updateOverlay() {
    ...
    ui->text("github.com/meta-flutter");
    ...
    if (ui->header("Scene")) {
        ...
        if (ui->button("Open gltf file")) {
            ...
            char buffer[1024];
            FILE *file = popen(
                    "zenity --title=\"Select a glTF file to load\" --file-filter=\"glTF files | *.gltf *.glb\" --file-selection",
                    "r");
            if (file) {
    ...
    if (imDrawData) {
        ...
        bool updateBuffers =
                (ui->vertexBuffer.buffer == VK_NULL_HANDLE) || (ui->vertexBuffer.count != imDrawData->TotalVtxCount) ||
                (ui->indexBuffer.buffer == VK_NULL_HANDLE) || (ui->indexBuffer.count != imDrawData->TotalIdxCount);
        ...
        if (updateBuffers) {
            ...
            ui->indexBuffer.create(vulkanDevice, VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT,
                                   indexBufferSize);
...
void VulkanPbr::render() {
    ...
    camera.rotationSpeed -= 0.0008f;
    if (camera.rotationSpeed < 0.0)
...
void VulkanPbr::fileDropped(std::string filename) {
    vkDeviceWaitIdle(device);
    loadScene(filename);
    setupDescriptors();
    recordCommandBuffers();
