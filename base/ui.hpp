...
public:
    ...
    UI(vks::VulkanDevice *vulkanDevice, VkRenderPass renderPass, VkQueue queue, VkPipelineCache pipelineCache,
       VkSampleCountFlagBits multiSampleCount, std::string assetPath) {
        ...
        */
        ...
        delete[] fontAsset;
#else
        io.Fonts->AddFontFromFileTTF((assetPath + "/Roboto-Medium.ttf").c_str(), 16.0f);
#endif
        io.Fonts->GetTexDataAsRGBA32(&fontData, &texWidth, &texHeight);
        fontTexture.loadFromBuffer(fontData, texWidth * texHeight * 4 * sizeof(char), VK_FORMAT_R8G8B8A8_UNORM,
                                   texWidth, texHeight, vulkanDevice, queue);
        ...
        */
        ...
        pipelineCI.layout = pipelineLayout;
        shaderStages = {
                loadShader(device, assetPath + "/shaders/ui.vert.spv", VK_SHADER_STAGE_VERTEX_BIT),
                loadShader(device, assetPath + "/shaders/ui.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT)
        };
