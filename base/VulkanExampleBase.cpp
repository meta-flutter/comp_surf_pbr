...
debugMessageCallback(VkDebugReportFlagsEXT flags, VkDebugReportObjectTypeEXT objType, uint64_t srcObject,
...
VulkanExampleBase::VulkanExampleBase(void *native_window, const char *asset_path, uint32_t width_, uint32_t height_) {
...
#elif defined(VK_USE_PLATFORM_WAYLAND_KHR)
//TODO    initWaylandConnection();
    typedef struct {
        struct wl_display *wl_display;
        struct wl_surface *wl_surface;
    } wl;

    auto p = reinterpret_cast<wl *>(native_window);
    display = p->wl_display;
    surface = p->wl_surface;
#elif defined(VK_USE_PLATFORM_XCB_KHR)
    initxcbConnection();
#endif
...
void VulkanExampleBase::initVulkan() {
    ...
    */
    if (settings.validation) {
        vkCreateDebugReportCallback = reinterpret_cast<PFN_vkCreateDebugReportCallbackEXT>(vkGetInstanceProcAddr(
                instance, "vkCreateDebugReportCallbackEXT"));
        vkDestroyDebugReportCallback = reinterpret_cast<PFN_vkDestroyDebugReportCallbackEXT>(vkGetInstanceProcAddr(
...
/*static*/void VulkanExampleBase::keyboardKeymapCb(void *data,
                                                   struct wl_keyboard *keyboard, uint32_t format, int fd,
                                                   uint32_t size) {
}
...
void VulkanExampleBase::setupFrameBuffer() {
    ...
    */
    ...
    mem_alloc.memoryTypeIndex = vulkanDevice->getMemoryType(memReqs.memoryTypeBits,
                                                            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
